		...
		foreach($reps as $rep){
			if($crep = $model->getByField('rid', $rep['rid'])){
				...
				if(isset($settings['agent_face']) && $settings['agent_face'] != ''){
					$tmp_html['content'] = str_replace('id="text-face">_______________________________________________________', 'id="text-face"> '.$settings['agent_face'], $tmp_html['content']);
				}
				if(isset($settings['agent_basis']) && $settings['agent_basis'] != ''){
					$tmp_html['content'] = str_replace('id="text-foundation">____________________________________', 'id="text-foundation"> '.$settings['agent_basis'], $tmp_html['content']);
				}
				if(isset($settings['agent_short_name']) && $settings['agent_short_name'] != ''){
					$tmp_html['content'] = str_replace('<span class="inline" id="text-subscriber">___________________</span>', $settings['agent_short_name'], $tmp_html['content']);
				}
