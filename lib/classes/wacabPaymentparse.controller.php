            ...
            foreach($all_tr as $tr){
                ...
                foreach($all_td as $tds){
                    ...
                    if(isset($td[1])){
                        ...
                        $payments[] = array(
                            'date' => $str_date, 
                            'before' => str_replace(' ', '', str_replace(',', '.', substr($td[2], strpos($td[2], '>')+1, strpos($td[2], '</td>') - strpos($td[2], '>')-1))),
                            'pay' => str_replace(' ', '', str_replace(',', '.', strip_tags(substr($td[3], strpos($td[3], '>')+1, strpos($td[3], '</td>') - strpos($td[3], '>')-1)))),
                            'after' => str_replace(' ', '', str_replace(',', '.',substr($td[4], strpos($td[4], '>')+1, strpos($td[4], '</td>') - strpos($td[4], '>')-1))),
